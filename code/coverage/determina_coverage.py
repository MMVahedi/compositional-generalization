...
def main():
    ...
    train_triple2id = {parse(it["input_text"]): it["id"] for it in train}
    test_triple2id = {parse(it["input_text"]): it["id"] for it in test}

    id2test_idx = {it["id"]: i for i, it in enumerate(test)}
    ...
    if args.k_sweep:
        ...
        for k in tqdm(k_values, desc="K-sweep progress"):
            ...
            # Map triple -> component_id
            triple2comp = {}
            comp_id = 0
            for comp in nx.connected_components(G):
                for tr in comp:
                    triple2comp[tr] = comp_id
                comp_id += 1
            logging.info(f"Found {comp_id} connected components in substitution graph")

            # For each component, gather its train and test triples (and indices)
            comp2train_triples = defaultdict(list)
            comp2test_triples = defaultdict(list)

            for tr, cid in triple2comp.items():
                if tr in train_triple2id:
                    comp2train_triples[cid].append(tr)
                if tr in test_triple2id:
                    comp2test_triples[cid].append(tr)

            # For each test item, attach pointers to training examples in same component
            for cid, t_triples in comp2test_triples.items():
                # All training triples in this component
                train_triples_in_comp = comp2train_triples[cid]
                if not train_triples_in_comp:
                    # No training in this component: no coverage via training
                    continue

                # Flatten to training indices
                train_ids_in_comp = [train_triple2id[tr] for tr in train_triples_in_comp]

                for tr in t_triples:
                    test_id = test_triple2id[tr]  # stored test ID, not index
                    test_idx = id2test_idx[test_id]  # list index of that entry
                    it = test[test_idx]

                    # Full list of train indices that are k-connected to this test example
                    it["coverage_train_ids"] = train_ids_in_comp
        ...
        k_sweep_file = os.path.join("k_sweep_results",
                                    f"{args.data_dir.split('/')[-2]}{'_ground-truth' if args.ground_truth else ''}.json")
        ...
        coverage_threshold_counts = {i: 0 for i in range(21)}  # Count examples for each threshold
        ...
        threshold_test_file = os.path.join(args.data_dir,
                                           f"test_annotated{'_ground-truth' if args.ground_truth else ''}.json")
        ...
        annotated_file = os.path.join(args.data_dir,
                                      f"test_annotated{'_ground-truth' if args.ground_truth else '_full'}.json")
        ...
        if args.visualise:
            ...
            plot_save_dir = os.path.join('coverage_visualization',
                                         f"{args.data_dir.split('/')[-2]}_full_min{args.min_evidence}.html")
